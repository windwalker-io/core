		...
		if (!$this->pathRegistered || $refresh)
		{
			...
			if (!$package instanceof NullPackage)
			{
				...
				if ($this->name)
				{
					...
					$this->renderer->addPath($globalPath . '/' . $this->getName() . '/' . $locale, PriorityQueue::BELOW_NORMAL);

					if ($default != $locale)
					{
						$this->renderer->addPath($globalPath . '/' . $this->getName() . '/' . $default, PriorityQueue::BELOW_NORMAL);
					}
