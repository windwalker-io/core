...
const fusionNext = require("@windwalker-io/fusion-next");
...
function findModules(suffix = "") {
  ...
  const vendors = Object.keys(pkgJson["require"] || {}).concat(Object.keys(pkgJson["require-dev"] || {})).map((id) => `vendor/${id}/composer.json`).map((file) => loadJson(file)).filter((pkgJson2) => pkgJson2?.extra?.windwalker != null).map((pkgJson2) => {
    return pkgJson2?.extra?.windwalker?.modules?.map((module2) => {
      return `vendor/${pkgJson2.name}/${module2}/${suffix}`;
    }) || [];
  }).flat();
  return [...new Set(vendors)];
}
function uniqId(prefix = "", size = 16) {
  let id = node_crypto.randomBytes(size).toString("hex");
  if (prefix) {
    id = prefix + id;
  }
  return id;
...
function cloneAssets(patterns) {
  return fusionNext.callback((taskName, builder) => {
  ...
  builder.assetFileNamesCallbacks.push((assetInfo) => {
    ...
    for (const base in reposition) {
      if (match(fileName, base)) {
        return path.normalize(reposition[base] + path.relative(removeLastGlob(base), fileName)).replace(/\\/g, "/");
      }
    }
  });
}
...
function cssModulize(entry, dest) {
  return new CssModulizeProcessor(fusionNext.css(entry, dest));
}
  ...
  config(taskName, builder) {
    ...
    for (const task of tasks) {
      builder.loadCallbacks.push((src, options) => {
        ...
        if (path.normalize(file) === path.resolve(task.input)) {
          const patterns = fastGlob.globSync(
            this.cssPatterns.map((v) => path.resolve(v)).map((v) => v.replace(/\\/g, "/"))
          );
...
function parseStylesFromBlades(patterns) {
  let files = fastGlob.globSync(patterns);
  return files.map((file) => {
    const bladeText = fs$1.readFileSync(file, "utf8");
    const html = nodeHtmlParser.parse(bladeText);
...
function installVendors(npmVendors = [], to = "www/assets/vendor") {
  return fusionNext.callbackAfterBuild(() => findAndInstall(npmVendors, to));
}
...
async function doInstall(source, dest) {
  if (process.env.INSTALL_VENDOR === "hard") {
    await fusionNext.copyGlob(source + "/**/*", dest);
  } else {
    await fusionNext.symlink(source, dest);
  }
}
...
exports.cloneAssets = cloneAssets;
exports.containsMiddleGlob = containsMiddleGlob;
exports.cssModulize = cssModulize;
exports.ensureDirPath = ensureDirPath;
exports.findModules = findModules;
exports.installVendors = installVendors;
exports.loadJson = loadJson;
exports.removeLastGlob = removeLastGlob;
exports.uniqId = uniqId;
exports.windwalkerAssets = windwalkerAssets;
