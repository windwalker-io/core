...
import { basename, parse, resolve, normalize, dirname, relative, isAbsolute } from 'node:path';
...
class ConfigBuilder {
  ...
  addWatch(file, watch) {
    this.watches = this.watches.filter((watch2) => {
      if (watch2 === file) {
        return false;
      }
      return !(typeof watch2 === "object" && watch2.file === file);
    });
  ...
  return [
    ...
    },
    {
      ...
      async writeBundle(options, bundle) {
        ...
        await moveFilesAndLog(builder.moveTasks, outDir2, logger);
        await copyFilesAndLog(builder.copyTasks, outDir2, logger);
        await linkFilesAndLog(builder.linkTasks, outDir2, logger);
    ...
    },
    {
      ...
      async transform(code, id) {
        ...
        function realpathToUrl(path2, base = "") {
          return normalizePath(path2).replace(normalizePath(serverRoot), base);
        }
...
function handleCustomWatchReload(server, watchTask, logger) {
...
export { alias, builder, callback, callbackAfterBuild, chunkDir, clean, configureBuilder, copy, copyGlob, css, index as default, externals, fileToId, fullReloads, getGlobBaseFromPattern, isDev, isProd, isVerbose, isWindows, js, link, move, moveGlob, outDir, overrideOptions, overrideViteConfig, params$1 as params, plugin, shortHash, symlink, useFusion };
