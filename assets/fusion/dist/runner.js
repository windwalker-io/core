...
import { resolveTaskOptions as resolveTaskOptions$1 } from '@/runner/config';
...
async function watchAll(optionsList) {
    const watcher = watch(optionsList.map((options) => ({
        ...options,
        watch: {
        // clearScreen: true,
        // buildDelay: 50
...
async function resolveTaskAsFlat(name, task, cache) {
    ...
    if (Array.isArray(task)) {
        ...
        cache[name] = task;
        const resolved = await resolveTaskOptions$1(task, true);
